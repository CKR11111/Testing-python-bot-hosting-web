...
def upload():
    ...
    if not f or not f.filename.lower().endswith(".zip"): return "Invalid", 400
    ...
    save_meta(meta)
    if auto: return redirect(url_for("start", bot=bid))
...
def start(bot):
    ...
    # Native subprocess instead of tmux
    ...
    while True:
        for bid, info in list(meta.items()):
            if info.get("auto") and not is_running(bid):
                folder = os.path.join(BOT_ROOT, bid)
                log = open(os.path.join(folder, "run.log"), "a")
                running_processes[bid] = subprocess.Popen(["python3", "ckr.py"], cwd=folder, stdout=log, stderr=log, start_new_session=True)
        time.sleep(10)
